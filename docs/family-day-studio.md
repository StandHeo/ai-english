...
5. 可按关用中英文编辑场景主题，再「配图本关」或「云端配全部关」；**每个槽**也可单独「云端配图」或「相册选图」（可混用，例如真实照片背景 + AI 道具）。相册图在本机压成 JPEG（最长边 768）并按槽裁切（背景 cover 竖屏、道具正方形），不保存相机原片。浏览器无原生相册时用文件选择。
...
- 配图较大时写入 IndexedDB，避免 `localStorage` 配额爆掉。相册选图与云端配图走同一套槽位存储。
- 相册选图 **不** 做抠图（透明底）；道具为正方形实底 JPEG。
- 不会改写官方水果/机器人等 pack。
