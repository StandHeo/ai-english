...
- [x] 1.1 增加 `002_email_auth.sql`：`users` 加可空唯一 `email`、`phone` 改为可空；新建 `auth_codes` 并迁出 `sms_codes`；`db.ts` 注册该 migration，测试能读到新列/新表且无 llm key 列
- [x] 1.2 扩展 `UserRow` 与 `getOrCreateUserByEmail` / `findUserByEmail`；短信路径的 `getOrCreateUser` 允许 `email` 为空；注销删除该用户 `auth_codes`；既有按号查找测试仍通过
...
- [x] 3.1 `GET /api/me` 返回掩码 `email` 与可选 `phone`；`POST /api/admin/plus` 接受 `email` 和/或 `phone`；API 测试覆盖邮箱登录、me、按邮箱开通、按号开通旧行
- [x] 3.2 admin 列表带 `email`/`emailMasked`；白名单含 `auth_codes`；控制台用户表与开通表单支持邮箱；相关 admin 测试更新并通过
