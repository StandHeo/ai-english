...
export function FamilyStudioPage() {
  ...
  const sceneLocked = Boolean(activeJob?.kind === 'image' || (activeJob?.kind === 'translate' && activeJob.phase === 'running'))
  ...
  const busy = generating // legacy alias for generate-only locks in a few call sites
  ...
  function finishJob(label: string, phase: 'done' | 'error' = 'done', holdMs = 2000) {
  ...
  async function requestMiniLevelImages(
    ...
    if (llmKey) {
      ...
      if (needTl.length) {
        showToast(`场景词翻译成英文中（${needTl.length} 关）…`)
  ...
  async function generate(force = false) {
    ...
    if (!story.trim()) {
      showToast(isToday ? '请先发几条今日故事（打字或语音）' : `这一天（${date}）没有日记故事，无法重新生成；可回到今天补记`)
      return
    }
    ...
    try {
      ...
      // autoFill 可能已启动配图 job；仅清除仍在跑的生成任务
      setActiveJob((prev) =>
  ...
  async function translateOneScene(levelId: string) {
    ...
    startJob('translate', '场景词翻译中…')
    updateJobLabel(`场景词翻译成英文中：${src.slice(0, 24)}…`)
    try {
  ...
  async function translateAllScenes() {
    ...
    clearJobNow()
    showToast(`翻译完成：${okCount}/${needTl.length} 关（配图时优先用英文场景词）`)
  }
  ...
  return (
    <div className={`family-studio chat-mode${showJobBanner ? ' has-job-banner' : ''}`}>
      {showJobBanner && (
        <div className={`studio-job-banner kind-${bannerKind}`} role="status" aria-live="polite">
          ...
          {messages.map((m) => (
            ...
            >
              ...
              {editingId === m.id ? (
                ...
                <>
                  <p className="bubble-text">{m.text || (m.audioId || m.audioDataUrl ? (pendingAsrIds[m.id] ? '（转写中…）' : '（待填写文字）') : '')}</p>
      ...
      <section className="generate-panel">
                  ...
                  >
                    ...
                    <div className="row">
                      <button
                        type="button"
                        className="ghost"
                        disabled={imageOpsLocked || sceneLocked}
        ...
        {hasLevel && miniLevels.length === 0 && (
          <div className="photo-block">
            ...
            />
            <div className="row photo-actions">
              <button type="button" disabled={imageOpsLocked || sceneLocked} onClick={() => fileRef.current?.click()}>
                从相册选图
              </button>
            </div>
            {images.length > 0 && (
